...
class Node:
    ...
    def updatef(self):
        self.f = self.g + self.h # TODO: attempt heuristic value set to 1.2 - need to determine whether optimal path provided
...
def checkfwd(node, nextloc):
    if node.prevaction[0] == 'S':
        ...
        nextloc.append((node.row + 12, node.col, 'n', 'W060'))
        ...
        nextloc.append((node.row, node.col + 12, 'e', 'W060'))
        ...
        nextloc.append((node.row - 12, node.col, 's', 'W060')) 
    elif node.direction == 'w':
        ...
        nextloc.append((node.row, node.col - 12, 'w', 'W060'))
...
def checkrev(node, nextloc):
    if node.prevaction[0] == 'W':
        ...
        nextloc.append((node.row - 12, node.col, 'n', 'S060'))
        ...
        nextloc.append((node.row, node.col - 12, 'e', 'S060'))
        ...
        nextloc.append((node.row + 12, node.col, 's', 'S060'))
        ...
        nextloc.append((node.row, node.col + 12, 'w', 'S060'))
