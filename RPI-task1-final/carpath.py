...
class Node:
    ...
    def updatef(self):
        self.f = self.g + 20*self.h # TODO: attempt heuristic value set to 5 - need to determine whether optimal path provided
...
def convertToRobot(pathList): 
    ...
    for i in range(len(pathList)):
        if pathList[i][0] == 'W':
            ...
            instruction.append("W009")
            instruction.append("K090")
            instruction.append("S006")
        elif pathList[i] == 'L':
            instruction.append("W009")
...
def instToFollow(nodePath, map, destination):
    ...
    skip = 0
    maxLen = len(nodePath)-1

    for i in range(len(nodePath)-1):
        if i+1+skip > maxLen:
            break
        print('Moving from: ', destination[nodePath[i-skip]], ' -> ', destination[nodePath[i+1]])
        path = findpath(map, destination[nodePath[i-skip]], destination[nodePath[i+1]])
        print(path)
        if path:
...
def pathFinder(pathFromAndroid, map):
    ...
    print("inside car path,pathFinder")
    print(destination)
